...
enum ast_pbx_result ast_pbx_start(struct ast_channel *c)
{
	...
	/* Fix for segfault in pthread_create */
	attr = ast_malloc(sizeof(pthread_attr_t)); /* Malloc attr space instead of using stack frame */
	if(!attr){
		ast_log(LOG_WARNING, "Malloc failed in file %s at line %d",__FILE__,__LINE__);
		return AST_PBX_FAILED;
	}
