			     ...
			     int line, const char *start_fn)
{
	...
	if (!attr) {
		/* Fix for segfault on an incoming iax call */
		attr = ast_malloc(sizeof(*attr)); /* This was an alloca in asterisk 1.4.19 */
		if(!attr){
			ast_log(LOG_WARNING, "Malloc failed in %s at line %d",__FILE__,__LINE__);
			return -1;
		}
		imalloced=1;
	...
	res = pthread_create(thread, attr, start_routine, data); /* We're in ast_pthread_create, so it's okay */
	if(imalloced)
		ast_free(attr);
	return res;
}
